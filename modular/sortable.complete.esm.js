...
function MultiDragPlugin() {
  function MultiDrag(sortable) {
    ...
    if (sortable.options.group) {
      const group = typeof sortable.options.group === 'string' ? {
        name: sortable.options.group
  ...
  MultiDrag.prototype = {
    ...
    _deselectMultiDrag(evt) {
      ...
      if (evt) {
        ...
        if (evt.button !== 0) return; // Only deselect if target is not item in any sortable in group (including this)
